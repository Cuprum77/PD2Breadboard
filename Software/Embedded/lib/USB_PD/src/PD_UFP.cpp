...
{
    // Initialize FUSB302
    gpio_init(PIN_FUSB302_INT);           // Set output enable pin as GPIO
    gpio_set_dir(PIN_FUSB302_INT, GPIO_IN); // Set output enable pin as output
    gpio_pull_up(PIN_FUSB302_INT);          // Set output enable pin pull up
...
{
    i2c_write_blocking(I2C_PORT, dev_addr, &reg_addr, 1, true);
    i2c_write_blocking(I2C_PORT, dev_addr, data, count, false);
...
void PD_UFP_log_c::print_status(void)
{
